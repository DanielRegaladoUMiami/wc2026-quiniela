// Pure type + helper module — safe to import from both server and client components.
...
export function averageRPS(rows: BacktestRow[]): number {
  ...
  return rows.reduce((acc, r) => acc + rps([r.p_home, r.p_draw, r.p_away], r.outcome), 0) / rows.length;
}

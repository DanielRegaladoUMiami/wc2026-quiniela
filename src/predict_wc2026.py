...
from src.models.stacker import load as load_stacker
...
def build_predictor(
    ...
    venue_country_by_pair = {
        (r["home_team"], r["away_team"]): r.get("venue_country")
        for _, r in features_wc.iterrows()
        if pd.notna(r.get("home_team"))
    }
    ...
    def predict(home: str, away: str) -> np.ndarray:
        ...
        if bay_model is not None:
            try:
                ...
                bay_pred = bay.predict(
                    bay_model, home, away, venue_country=vc, max_goals=sm.shape[0] - 1
                )

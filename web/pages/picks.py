...
def _1x2_table(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for r in rows.itertuples():
        triples = [("1 " + r.home, r.p_home_win), ("X Draw", r.p_draw), ("2 " + r.away, r.p_away_win)]
        pick, p = max(triples, key=lambda x: x[1])
        picks.append(pick); conf.append(p)
    return pd.DataFrame({
...
def _exact_table(rows: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame({
        ...
        "Pick (mode)": [f"{round(r.expected_home_goals)}-{round(r.expected_away_goals)}"
                        for r in rows.itertuples()],
    })
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Bracket"):
            ...
            if adv is None:
                ...
                gr.Dataframe(top[["team", "P(Champion)"]].rename(columns={"team": "Team"}),
                             interactive=False)

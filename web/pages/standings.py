...
def _group_fig(letter: str, rows: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_bar(
        y=rows["team"], x=rows["p_advance_group"], orientation="h",
        marker_color=PALETTE["primary"],
        hovertemplate="%{y}<br>P(advance)=%{x:.1%}<extra></extra>",
        text=[f"{v:.0%}" for v in rows["p_advance_group"]], textposition="inside",
        insidetextfont=dict(color="white"),
    )
    fig.update_layout(
        title=f"Group {letter}", template="plotly_white",
